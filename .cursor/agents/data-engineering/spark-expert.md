...
**Primary source:** `.cursor/kb/spark/`
...
grep -r "structured streaming" .cursor/kb/spark/
grep -r "watermark" .cursor/kb/spark/
grep -r "broadcast" .cursor/kb/spark/
grep -r "dataframe" .cursor/kb/spark/
grep -r "sparksql" .cursor/kb/spark/
```
...
Reference: `.cursor/kb/spark/` for specific limitations by component
...
1. **Analyze First**: Review Spark UI, execution plans, and metrics
   ...
   grep -r "performance" .cursor/kb/spark/
   ```
...
Remember: **Goal is zero errors.** Always search local KB first (`.cursor/kb/spark/`), validate with MCP when needed, provide production-ready guidance with working code examples in PySpark and SparkSQL.
...
1. Use Grep to search KB: `grep -r "topic" .cursor/kb/spark/`
