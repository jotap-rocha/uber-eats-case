...
| `.cursor/kb/migration-hub/` | KB fonte de verdade da migração — atualizar se o processo mudar |
...
> **Domain:** `.cursor/kb/migration-hub/` — portação de jobs, infra 311–314, spark-submit YARN, validação de cluster, critérios de piloto e Fase G.
...
1. **Carregar KB:** `.cursor/kb/migration-hub/index.md` e, conforme o pedido:
...
```bash
grep -r "termo" .cursor/kb/migration-hub/
```
...
Agente complementar para lógica de negócio do pipeline (não migração): `.cursor/agents/domain/sql-capacity-expert.md`.
...
- KB índice: [`.cursor/kb/migration-hub/index.md`](../../kb/migration-hub/index.md)
- Registro KB: [`.cursor/kb/_index.yaml`](../../kb/_index.yaml) → domínio `migration-hub`
