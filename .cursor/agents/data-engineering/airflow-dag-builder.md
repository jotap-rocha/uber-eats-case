...
description: |
  ...
  assistant: "Vou usar o airflow-dag-builder: inspecionar o repo, confirmar dag_id, tasks, upstreams explícitos e gerar o grafo TaskFlow conforme `.cursor/kb/airflow`."
  <commentary>
...
Quando uma entrega alterar **onde DAGs vivem**, convenções de `dag_id` ou o contrato de tasks deste agente, atualize ou proponha atualização de `.cursor/commands/core/router.md`, `HOW_TO_USE_AGENTS.md` e, se aplicável, `.cursor/kb/airflow/` ou **migration-hub** (`data-stack/airflow`).
...
│  6. KB ALIGN  → `.cursor/kb/airflow` — TaskFlow, idempotência, 3 fases      │
...
| Prioridade | Path |
|------------|------|
| Índice | `.cursor/kb/airflow/index.md` |
| Referência rápida | `.cursor/kb/airflow/quick-reference.md` |
| TaskFlow | `.cursor/kb/airflow/concepts/taskflow-api.md` |
| Idempotência / macros | `.cursor/kb/airflow/concepts/idempotencia-macros-templates.md` |
| Pré / carga / pós-check | `.cursor/kb/airflow/patterns/pipeline-pre-check-load-post-check.md` |
| Retries / callbacks | `.cursor/kb/airflow/patterns/retries-callbacks-alertas.md` |
| Dynamic mapping (várias entidades) | `.cursor/kb/airflow/patterns/dynamic-task-mapping.md` |
| Tags obrigatórias (área, stack, frequência) | `.cursor/kb/airflow/concepts/dag-metadata-tags.md` |
| **Alerta Teams (falha de task)** | `/DATALAKE/dags/utils/send.py` — `failure(context)` |
| Infra edge 314, pasta DAGs | `.cursor/kb/migration-hub/data-stack/airflow/README.md` |

Apoio para **ambiente DATALAKE** (paths, utilizadores): `.cursor/agents/data-engineering/big-data-cluster-expert.md`.
...
Detalhe e vocabulário: `.cursor/kb/airflow/concepts/dag-metadata-tags.md`.
...
- siga o pattern **pós-check** em `.cursor/kb/airflow/patterns/pipeline-pre-check-load-post-check.md`.
...
Se a política do projeto for **falhar** a task quando invariantes não batem, usar `raise AirflowException` e **retries** conforme `.cursor/kb/airflow/patterns/retries-callbacks-alertas.md`.
