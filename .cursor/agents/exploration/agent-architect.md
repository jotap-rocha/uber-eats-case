...
description: |
  Especialista em projetar e implementar novos arquivos de agente Cursor (Markdown em `.cursor/agents/`).
...
> **Não é:** `prompt-crafter` (gera `PROMPT.md` para Dev Loop) nem `kb-architect` (estrutura `.cursor/kb/`).
...
│  4. AUTHOR     → Escrever `.cursor/agents/<categoria>/<nome>.md`           │
...
| Recurso | Uso |
|---------|-----|
| [`.cursor/agents/_template/agent.template.md`](../_template/agent.template.md) | Estrutura, frontmatter, checklist |
...
| [`.cursor/commands/core/router.md`](../../commands/core/router.md) | Onde e como registrar roteamento |
...
1. **`name` no frontmatter** deve ser único em `.cursor/agents/` (validar com `Grep`).
2. **Caminho do arquivo:** `.cursor/agents/<categoria>/nome-kebab.md` — categorias usuais: `workflow`, `dev`, `data-engineering`, `code-quality`, `ai-ml`, `communication`, `exploration`, `domain`.
...
| Artefato | Ação |
|----------|------|
| [`.cursor/commands/core/router.md`](../../commands/core/router.md) | Incluir na lista *Agentes registrados* (secção Exploration ou outra adequada); adicionar subsecção “Seleção principal” se o agente for roteado com frequência |
...
- Aplicar as mesmas entradas em [`.github/commands/core/router.md`](../../../.github/commands/core/router.md) / [`.claude/commands/core/router.md`](../../../.claude/commands/core/router.md) (trocar prefixo `.cursor/agents` → `.github/agents` ou `.claude/agents` nas linhas do router espelho).

### 6. Verificação

```bash
# Nome duplicado no frontmatter
rg '^name:' .cursor/agents --glob '*.md'

# Referências ao novo agente
rg 'nome-do-agente' .cursor/commands/core HOW_TO_USE_AGENTS.md
```
...
| Misturar escopo com `prompt-crafter` | PROMPT.md ≠ agente em `.cursor/agents/` |
...
| Estruturar `.cursor/kb/` | `kb-architect` |
...
```text
[ ] Arquivo criado em .cursor/agents/<categoria>/<nome>.md
...
> **Missão:** cada agente novo deve ser **encontrável**, **roteável** e **consistente** com o restante de `.cursor/agents/`.
