...
Atualizar `router.md`, `.cursor/CURSOR.MD`, `HOW_TO_USE_AGENTS.md` e `AGENT_ROUTER.yaml` quando mudar critérios de classificação ou KBs de layout.
...
Em projetos de **dados** (ingestão multi-fonte), **não** tratar **`src/app`** como padrão de automação: aplicar a árvore **`.cursor/kb/design-pipelines-patterns/`** — `etl/` (extract/transform/load desacoplados), `pipelines/` (orquestração por fonte), `src/` (partilhado, com **`src/utils/logger.py`** e **`src/utils/key_oci.py`** obrigatórios), `sql/`, `tests/`, `config/`, `data/`, `logs/`, `docs/`. Complementar com **`.cursor/kb/dev-python-engineering/`**. Repositórios legados com apenas `src/jobs/` podem migrar gradualmente para esta árvore.
...
|------|----------------|---------------------|-----|
| **Dados** | Pipelines, ingestão **por fonte**, ELT, vários feeds/APIs/ficheiros com regras diferentes | `etl/`, `pipelines/` (**um módulo por fonte**), `src/` (`contracts/`, `validation/`), `sql/`, `tests/`, `config/`, `data/`, `logs/`, `docs/`. **Não** impor **`src/app`**. | `.cursor/kb/design-pipelines-patterns/` · `.cursor/kb/dev-python-engineering/` |
| **Automação** | Job operacional único, orquestração batch, integrações encadeadas sob um bootstrap | `src/app/main.py`, `src/core/`, `src/integrations/` | `.cursor/kb/design-automation-project-patterns/` · `.cursor/kb/dev-python-automation/` |
...
| Domínio | Entrada |
|---------|---------|
| Layout projetos **dados** | [`.cursor/kb/design-pipelines-patterns/index.md`](../../kb/design-pipelines-patterns/index.md) |
| Layout projetos **automação** | [`.cursor/kb/design-automation-project-patterns/index.md`](../../kb/design-automation-project-patterns/index.md) |
| Engenharia Python (dados) | [`.cursor/kb/dev-python-engineering/index.md`](../../kb/dev-python-engineering/index.md) |
| Automação / SQL Server | [`.cursor/kb/dev-python-automation/index.md`](../../kb/dev-python-automation/index.md) |
| Cluster / migração | [`.cursor/kb/migration-hub/index.md`](../../kb/migration-hub/index.md) |
