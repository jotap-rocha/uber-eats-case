...
Atualizar `router.md`, `.cursor/CURSOR.MD` e KB quando o padrão **spark-submit** ou roteamento de agentes mudar.
...
| Caminho | Uso |
|---------|-----|
| `.cursor/kb/migration-hub/index.md` | Ordem de leitura cluster |
| `.cursor/kb/migration-hub/spark/PADRAO-SPARK-SUBMIT-PROJETO-PRD.md` | **Padrão obrigatório** spark-submit |
| `.cursor/kb/migration-hub/spark/01-spark-3.5.2.md` | Spark 3.5.2 on YARN |
| `.cursor/kb/migration-hub/spark/CHECKLIST-BOAS-PRATICAS.md` | Smoke e boas práticas |
| `.cursor/agents/data-engineering/big-data-cluster-expert.md` | Âncora operacional cluster |

### Layout e código

| Caminho | Uso |
|---------|-----|
| `.cursor/kb/design-pipelines-patterns/index.md` | Árvore `etl/`, `pipelines/`, `config/` |
| `.cursor/kb/design-pipelines-patterns/patterns/spark-submit-yarn-config.md` | Onde guardar `spark.conf` |
| `.cursor/kb/design-pipelines-patterns/specs/project-layout-mandatory.yaml` | Layout máquina-legível |

### Referência técnica Spark

| Caminho | Uso |
|---------|-----|
| `.cursor/kb/spark/index.md` | Índice APIs e guias |
| `.cursor/kb/spark/quick-reference.md` | Consulta rápida |

Registo KB: `.cursor/kb/_index.yaml` → `migration-hub`, `design-pipelines-patterns`, domínio `spark/` (pasta).
