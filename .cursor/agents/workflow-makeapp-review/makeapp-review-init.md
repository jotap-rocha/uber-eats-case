...
description: |
  ...
  sob .cursor/sdd/reviews/{RUN_ID}/. Não executa análise profunda — só estrutura + próximos passos.
...
> **Domain:** `.cursor/sdd/reviews/` · `WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml`  
> **Entrada do fluxo (usuário):** `/workflow-makeapp-review` — `.cursor/commands/workflow-makeapp-review/workflow-makeapp-review.md`
...
2. **Paths** — `root = .cursor/sdd/reviews/{RUN_ID}/`, `lanes/ = root/lanes/`.
3. **Copiar** conteúdo base do template [`.cursor/sdd/reviews/_template/REVIEW_MANIFEST.yaml`](../../sdd/reviews/_template/REVIEW_MANIFEST.yaml): substituir `REPLACE_RUN_ID`, `REPLACE_ISO8601` (UTC).
...
[ ] Visão geral em `.cursor/commands/workflow-makeapp-review/workflow-makeapp-review.md`; detalhes em `reviewer-lane.md`
```
