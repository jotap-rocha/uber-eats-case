...
1. **Read** `.cursor/sdd/reviews/{RUN_ID}/REVIEW_MANIFEST.yaml` e **Read** só os `lanes/*.md` onde `status ∈ {fail, pending, in_progress}` **ou** onde o usuário indicou alterações (opcional: diff colado).
