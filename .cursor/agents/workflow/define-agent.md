...
| **Output** | `.cursor/sdd/features/DEFINE_{FEATURE}.md` |
...
```markdown
Read(.cursor/sdd/templates/DEFINE_TEMPLATE.md)
Read(.cursor/CURSOR.MD)
...
(Select all that apply - reference .cursor/kb/_index.yaml)
...
### 3.2 Pipeline data mandates (OBRIGATÓRIO se pipeline Databricks/Spark)

Se o input indicar **ingestão/transformação de dados** (medalhão, DLT, Auto Loader, Spark ETL, Unity Catalog):

1. **Ler** `.cursor/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml`
2. **Verificar** `applies_when.signals` — se aplicável, **injetar** secção no DEFINE mesmo que o analista não peça
3. **Anexar** checklist de `.cursor/sdd/templates/PIPELINE_OPS_REQUIREMENTS_APPENDIX.md`
4. **Perguntar** (se ainda em falta) via `AskUserQuestion`:
   - Canal de alertas operacionais (Teams/email)?
   - Origem com schema variável (CSV SI)? Política drift?
   - Política para registos inválidos (quarentena vs drop)?
   - SLA freshness dos dados (sentinela)?
   - Política reprocessamento e quarentena histórica?
5. Marcar itens injectados com flag `injected_by_pipeline_mandate`

**Secção no DEFINE:** `## Requisitos operacionais de pipeline (mandato)` — cada item DC-M*, DQ-M*, SD-M*, TM-M*, OBS-M* como MUST/SHOULD/N/A.

**KB:** `.cursor/kb/data-engineering-practices/index.md`

### 3.3 PyODBC — escrita SQL Server (gate humano OBRIGATÓRIO)

Se o input indicar **PyODBC**, **INSERT no SQL Server** ou destino relacional ODBC:

1. **Ler** `.cursor/kb/data-engineering-practices/pyodbc-sqlserver-write-mode.md` e `PIPELINE_MANDATORY_PRACTICES.yaml` → `pyodbc_human_gate`
2. **Usar `AskUserQuestion`** — **não assumir defaults**:
   - **Pergunta 1:** *APPEND ou Overwrite?* Explicar implicações (ver KB).
   - **Pergunta 2** (só se Overwrite): *`truncate=true` ou `truncate=false`?* Recomendar **`false`** (mantém DDL/tipos; evita drop+recreate STRING).
3. **Preencher** `.cursor/sdd/templates/PYODBC_WRITE_MODE_GATE.md` no DEFINE (secção `## PyODBC — modo de escrita SQL Server`)
4. **Não avançar** para Design com clareza ≥80% se PyODBC estiver no escopo e gate incompleto
5. Se Overwrite + `truncate=true` em PRD → registar como risco + exigir aprovação DBA no DEFINE
...
- [ ] **Se pipeline de dados:** secção mandato presente (PIPELINE_MANDATORY_PRACTICES) ou N/A justificado
- [ ] **Se PyODBC → SQL Server:** gate APPEND/Overwrite + truncate documentado (PYODBC_WRITE_MODE_GATE)
...
- Command: `.cursor/commands/workflow/define.md`
- Template: `.cursor/sdd/templates/DEFINE_TEMPLATE.md`
- Contracts: `.cursor/sdd/architecture/WORKFLOW_CONTRACTS.yaml`
- Previous Phase: `.cursor/agents/workflow/brainstorm-agent.md` (optional)