...
| **Output** | `.cursor/sdd/features/BRAINSTORM_{FEATURE}.md` |
...
Read(.cursor/sdd/templates/BRAINSTORM_TEMPLATE.md)
Read(.cursor/kb/_index.yaml)  # Available KB domains
Glob(.claude/skills/*/SKILL.md)  # Available Skills — ver .cursor/rules/skill-first.mdc
...
- Skills disponíveis → Se já existe uma Skill cobrindo "como fazer X", preferir referenciá-la em
  vez de desenhar um novo padrão de KB do zero (ver `.cursor/rules/skill-first.mdc`)
...
**Se ETL/Databricks/Spark/medalhão** — incluir perguntas operacionais (uma de cada vez):

| Tema | Pergunta |
|------|----------|
| Alertas | Teams, email ou outro canal operacional? |
| Schema origem | Cabeçalhos/colunas podem mudar (schema drift)? |
| Inválidos | Quarentena auditável, drop ou fail? |
| Freshness | SLA de dados frescos na camada serving? |
| Reprocessamento | Re-run manual é cenário normal? |

Registar na secção **Operational context** do BRAINSTORM → alimenta mandatos em `/define` (`PIPELINE_MANDATORY_PRACTICES.yaml`).

**Se PyODBC / SQL Server destino** — perguntar cedo (antes do design):

| Tema | Pergunta |
|------|----------|
| Modo carga | APPEND (acrescenta) ou Overwrite (substitui)? |
| TRUNCATE | Se Overwrite: truncate=false (recomendado) ou true (risco drop/recreate)? |

Registar respostas para o Define completar `PYODBC_WRITE_MODE_GATE.md`.
...
2. Save document to `.cursor/sdd/features/BRAINSTORM_{FEATURE}.md`
3. Inform user: "Ready for `/define .cursor/sdd/features/BRAINSTORM_{FEATURE}.md`"
...
- Command: `.cursor/commands/workflow/brainstorm.md`
- Template: `.cursor/sdd/templates/BRAINSTORM_TEMPLATE.md`
- Contracts: `.cursor/sdd/architecture/WORKFLOW_CONTRACTS.yaml`
- Next Phase: `.cursor/agents/workflow/define-agent.md`