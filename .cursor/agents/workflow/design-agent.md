...
| **Phase** | 2 - Design |
| **Input** | `.cursor/sdd/features/DEFINE_{FEATURE}.md` |
| **Output** | `.cursor/sdd/features/DESIGN_{FEATURE}.md` |
...
### 5.0 Pipeline operational design (se DEFINE indica pipeline Databricks/Spark)

1. **Ler** `.cursor/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml` → `sdd_design_injection.file_manifest_hints`
2. **Incluir** no manifest **todos** os componentes aplicáveis (Teams, quarentena, drift, sentinela, contrato, testes)
3. **Anexar** `.cursor/sdd/templates/PIPELINE_OPS_DESIGN_APPENDIX.md` como secção `## Design operacional — componentes mandatórios`
4. **Documentar ADRs** inline: idempotência, schema drift, retenção quarentena, canal alertas
5. **Atribuir agentes:** `@databricks-pipeline-architect`, `@teams-notifications-expert`, `@data-engineering-reviewer`

### 5.0.1 PyODBC → SQL Server (se aplicável)

Se DEFINE ou requirements mencionam **PyODBC** / INSERT SQL Server:

1. Confirmar gate preenchido (`PYODBC_WRITE_MODE_GATE.md`)
2. Design deve reflectir **modo** e **truncate** nos parâmetros de carga (config YAML/código)
3. ADR inline obrigatório: APPEND vs Overwrite; se Overwrite, por que `truncate=false` (ou excepção)
4. Testes: re-run DSV — APPEND duplica? Overwrite idempotente?
...
| Skills disponíveis (`.claude/skills/`) | Medium | Skill já cobre "como fazer X" → referenciar em vez de desenhar do zero (ver `.cursor/rules/skill-first.mdc`) |
...
- [ ] **Se pipeline de dados:** manifest inclui contrato, Teams, quarentena, testes; drift/sentinela se aplicável
...
- Command: `.cursor/commands/workflow/design.md`
- Template: `.cursor/sdd/templates/DESIGN_TEMPLATE.md`
