...
| Arquivo | Finalidade |
|---------|------------|
| `.cursor/commands/core/router.md` | Roteamento slides Icatu (builder + reviewer) |
| `HOW_TO_USE_AGENTS.md` | Tabela `communication/` |
| `.cursor/kb/icatu-slides/` | Evolução das regras — alinhar este agente se a KB mudar |
...
Ficheiros em **`.cursor/kb/icatu-slides/`**:
...
**Verdict:** PASS | BLOCK  
**Next step:** Correções **mecânicas** (hex, acentos, eventos JS, substituições pontuais) → `@.cursor/agents/communication/icatu-slide-fixer.md`. **Estrutura / slides novos / barra completa** → `@.cursor/agents/communication/icatu-slide-builder.md` ou humano. Depois, **reexecutar este reviewer** no mesmo HTML.
```
