...
description: |
  ...
  sob .cursor/sdd/data-pipeline-reviews/{RUN_ID}/. Não executa análise profunda.
...
> **Domain:** `.cursor/sdd/data-pipeline-reviews/` · `WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml`  
...
2. **Paths** — `root = .cursor/sdd/data-pipeline-reviews/{RUN_ID}/`, `lanes/ = root/lanes/`.
3. Copiar template `.cursor/sdd/data-pipeline-reviews/_template/REVIEW_MANIFEST.yaml`, substituindo `REPLACE_RUN_ID`, `REPLACE_ISO8601` e `{BASE}`.
