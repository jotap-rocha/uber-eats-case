...
grep -r "window function" .cursor/kb/spark/
grep -r "adaptive query execution" .cursor/kb/spark/
```

### 3. Use the Agent

The Spark expert agent at `.cursor/agents/spark-expert.md` provides:
...
Search: grep -r "adaptive query execution" .cursor/kb/spark/
```

### 5. Troubleshooting

```
Search: grep -r "error message" .cursor/kb/spark/
