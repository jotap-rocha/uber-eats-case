...
Ponto de entrada quando trabalhas com **`.cursor/kb`** ou `@`. **Este directorio** (`.cursor/kb/migration-hub/`) concentra a documentação narrativa da migração e da stack; **código executável** (scripts de infra, dbt): **`/DATALAKE/workspace/jp/platform/`** (`scripts/`, `dbt/`).
