...
name: devloop-init
description: |
  Inicia execução Dev Loop (L2): cria RUN em .cursor/sdd/dev-loop-runs/{RUN_ID}/ com manifest,
  stubs de fases e pasta artifacts/. Não implementa código — use /devloop-phase por fase.
---

# `/devloop-init` — Dev Loop (passo 0)

**Canônico:** `.cursor/commands/workflow-dev-loop/devloop-init.md`  
...
1. Cria `.cursor/sdd/dev-loop-runs/{RUN_ID}/`
...
**`devloop-init`** — `.cursor/agents/workflow-dev-loop/devloop-init.md`
...
Contrato: `.cursor/sdd/architecture/WORKFLOW_DEV_LOOP_CONTRACTS.yaml`  
Documentação: `.cursor/sdd/dev-loop-runs/README.md`
