---
name: sync-context
description: Sync project context to .cursor/CURSOR.MD by analyzing codebase patterns and conventions
---

# Sync Context Command

Analyzes codebase and intelligently updates `.cursor/CURSOR.MD` with current project context.
...
3. **Merges** with existing `.cursor/CURSOR.MD` content
...
Generated `.cursor/CURSOR.MD` follows this structure:
...
```text
1. Read existing `.cursor/CURSOR.MD`
   │
   ▼
...
6. Validate (dry-run or save)
   │
   ▼
7. Write updated `.cursor/CURSOR.MD`
```
...
━━━━━━━━━━━━━━━━
`.cursor/CURSOR.MD` updated successfully
```
...
git diff .cursor/CURSOR.MD

# Commit with context
git add .cursor/CURSOR.MD
git commit -m "chore: update .cursor/CURSOR.MD with latest project structure"
```