...
2. consultar `.cursor/commands/core/router.md`
3. selecionar o agente primário mais apropriado
4. abrir e seguir o arquivo markdown do agente selecionado em `.cursor/agents/`
5. consultar a knowledge base relevante em `.cursor/kb/`, quando aplicável
...
- Sempre consultar `.cursor/commands/core/router.md` antes de responder
...
- Quando o projeto evoluir (novos agentes, novas prioridades), **atualizar** `.cursor/commands/core/router.md` para refletir a mudança
...
### Etapa 1 — roteamento
Leia `.cursor/commands/core/router.md` e determine:
...
Abra o arquivo do agente primário selecionado em `.cursor/agents/`.
...
- consulte `.cursor/kb/_index.yaml`
- carregue o domínio relevante em `.cursor/kb/`
...
- agente primário
- KB relevante
- contexto do projeto em `.cursor/CURSOR.MD` (se existir)
...
|----------|------------------|
| `.cursor/commands/core/router.md` | Novos `.md` em `.cursor/agents/`, mudança de prioridades ou de “agente expert” do repo |
| `.cursorrules`, `.cursor/CURSOR.MD`, `CONTEXT.md` | Mudança de comandos, estrutura ou convenções usadas no dia a dia |
