...
1. **Reads session data** from `.cursor/telemetry/sessions/`
2. **Computes aggregates** (phase timing, agent effectiveness, trends)
3. **Generates reports** in `.cursor/telemetry/reports/`
...
- `.cursor/sdd/archive/*/` - Shipped feature artifacts
- `.cursor/telemetry/sessions/` - Session JSON files
...
3. Save session JSON to `.cursor/telemetry/sessions/`
4. Update aggregates
...
- Delete anytime: `rm -rf .cursor/telemetry/`