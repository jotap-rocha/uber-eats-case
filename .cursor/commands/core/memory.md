...
Save session insights to `.cursor/storage/` for future reference.
...
3. **Saves** to `.cursor/storage/memory-{YYYY-MM-DD}.md`
...
Creates: `.cursor/storage/memory-{YYYY-MM-DD}.md`
...
3. Write to storage:
   - Create .cursor/storage/ if not exists
...
Saved to: .cursor/storage/memory-2026-01-23.md
