...
/define .cursor/sdd/features/BRAINSTORM_INVOICE_PROCESSING.md
...
Phase 1: /define     → .cursor/sdd/features/DEFINE_{FEATURE}.md (THIS COMMAND)
Phase 2: /design     → .cursor/sdd/features/DESIGN_{FEATURE}.md
Phase 3: /build      → Code + .cursor/sdd/reports/BUILD_REPORT_{FEATURE}.md
Phase 4: /ship       → .cursor/sdd/archive/{FEATURE}/SHIPPED_{DATE}.md
```
...
Read(.cursor/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml)  # se pipeline de dados

# If file provided:
Read(<input-file>)
```

**Pipeline Databricks/Spark:** se o input indicar ingestão/medalhão/DLT, injectar secção mandato via `PIPELINE_OPS_REQUIREMENTS_APPENDIX.md` mesmo que o analista omita.

**PyODBC → SQL Server:** se houver INSERT via PyODBC, **obrigatório** gate humano (`AskUserQuestion`): APPEND vs Overwrite; se Overwrite, truncate=false (recomendado) vs true. Ver `PYODBC_WRITE_MODE_GATE.md`.
...
```markdown
Write(.cursor/sdd/features/DEFINE_{FEATURE_NAME}.md)
```
...
| Artifact | Location |
|----------|----------|
| **DEFINE** | `.cursor/sdd/features/DEFINE_{FEATURE_NAME}.md` |

**Next Step:** `/design .cursor/sdd/features/DEFINE_{FEATURE_NAME}.md`
...
- Agent: `.cursor/agents/workflow/define-agent.md`
- Template: `.cursor/sdd/templates/DEFINE_TEMPLATE.md`
- Contracts: `.cursor/sdd/architecture/WORKFLOW_CONTRACTS.yaml`
- Previous Phase: `.cursor/commands/workflow/brainstorm.md` (optional)
- Next Phase: `.cursor/commands/workflow/design.md`