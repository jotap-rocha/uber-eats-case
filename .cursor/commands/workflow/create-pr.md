...
Para a disciplina de commit **antes** de chegar aqui (granularidade, quando commitar em cada fase SDD/Dev Loop,
o que nunca fazer sem autorização explícita), ver `.claude/skills/git-boas-praticas-versionamento/SKILL.md`.
...
## Pre-PR: outras pastas (`.claude/` e `.github/`)

Este fluxo trata **`.cursor/`** como referência para `agents/`, `kb/`, `commands/` e `sdd/`. Se o PR alterar também **`.claude/`** ou **`.github/`**, não há checagem automática de alinhamento entre árvores; alinhe **manualmente** se o time exigir conteúdo equivalente.

---

## Pre-PR Review Option

When using `--review`, the command runs a **dual AI review** (CodeRabbit + CURSOR) before creating the PR:
...
│          ↓                                                       │
│   3. Run CURSOR Review (architectural)                          │
│          ↓                                                       │
...
Use this mentalmente (ou cole na descrição do PR quando fizer sentido):

```text
[ ] Revisão: /review ou /create-pr --review (se política do time)
[ ] CONTEXT.md / .cursorrules / `.cursor/CURSOR.MD`: atualizados se mudou como se roda, depura ou usa agentes/comandos (veja Definition of Done em [`HOW_TO_USE_AGENTS.md`](../../../get_started/HOW_TO_USE_AGENTS.md) e `.cursor/commands/core/router.md`)
```
...
Co-Authored-By: CURSOR Opus 4.5 <noreply@anthropic.com>
```
...
Generated with [CURSOR Code](https://CURSOR.ai/code)
```
...
- Review Command: `.cursor/commands/review/review.md`
- Dual Reviewer Agent: `.cursor/agents/code-quality/dual-reviewer.md`
- CI: nenhuma configurada no momento (GitHub Actions pode ser adicionado em `.github/workflows/` se necessário)
- CodeRabbit Config: `.coderabbit.yaml`
- Template: `.github/PULL_REQUEST_TEMPLATE.md`
- Workflow: `.cursor/sdd/_index.md`
- Agents: `.cursor/agents/workflow/`