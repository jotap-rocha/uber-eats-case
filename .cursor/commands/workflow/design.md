...
```bash
/design .cursor/sdd/features/DEFINE_CLOUD_RUN_FUNCTIONS.md
/design DEFINE_USER_AUTH.md
/design .cursor/sdd/features/DEFINE_INVOICE_EXTRACTION.md
```
...
Phase 2: /design     → .cursor/sdd/features/DESIGN_{FEATURE}.md (THIS COMMAND)
...
Read(.cursor/sdd/templates/DESIGN_TEMPLATE.md)
Read(.cursor/CURSOR.MD)
Read(.cursor/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml)  # se pipeline de dados
Read(.cursor/sdd/templates/PIPELINE_OPS_DESIGN_APPENDIX.md)       # se pipeline de dados
Glob(.claude/skills/*/SKILL.md)  # Skills disponíveis — ver .cursor/rules/skill-first.mdc antes de propor KB novo
...
**Pipeline Databricks/Spark:** file manifest deve incluir componentes operacionais (contrato, Teams, quarentena, drift, sentinela, testes) conforme mandatos.
...
```markdown
Write(.cursor/sdd/features/DESIGN_{FEATURE_NAME}.md)
```
...
| Artifact | Location |
|----------|----------|
| **DESIGN** | `.cursor/sdd/features/DESIGN_{FEATURE_NAME}.md` |

**Next Step:** `/build .cursor/sdd/features/DESIGN_{FEATURE_NAME}.md`
