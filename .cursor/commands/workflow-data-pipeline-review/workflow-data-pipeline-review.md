...
description: |
  ...
  .cursor/sdd/data-pipeline-reviews/{RUN_ID}/.
---

# `/workflow-data-pipeline-review` — Data Pipeline Review (entrada)

**Arquivo canônico:** `.cursor/commands/workflow-data-pipeline-review/workflow-data-pipeline-review.md`.

Este comando é o **roteador** do review. Execute os passos abaixo na ordem, um slash por objetivo.

## Passos

1. **`/pipeline-review-init <RUN_ID>`** — Cria `.cursor/sdd/data-pipeline-reviews/{RUN_ID}/`, manifest, stubs de lanes e log inicial.  
   Ex.: `/pipeline-review-init pr-1842-pipeline-dim-cliente`

2. **`/pipeline-review-lane <RUN_ID> <lane_id>`** — Uma lane por invocação.  
   Ordem recomendada (ver [`.cursor/sdd/architecture/WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml`](../../sdd/architecture/WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml)):
   ...
   **Mandatos operacionais:** cruzar findings com [`.cursor/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml`](../../sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml) (Teams, schema drift, quarentena, sentinela, contrato).
...
Agentes: `.cursor/agents/workflow-data-pipeline-review/` · Índice: [`README.md`](README.md)
