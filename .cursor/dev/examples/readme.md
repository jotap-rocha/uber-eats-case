...
This repository now includes a project-specific first PROMPT:

```text
/dev tasks/PROMPT_HELLO_WORLD.md
```

Expected outcome: create `.cursor/dev/examples/HELLO_WORLD_RUN.md` and validate the native repository checks:
...
If the smoke run has not been executed yet, keep `HELLO_WORLD_RUN.md` as the placeholder artifact to be created by that PROMPT.
