...
Os exemplos com `@` abaixo usam o prefixo **`.claude/`** (referência principal do repositório). No **Claude Code** use `.claude/`; no **Copilot** use `.github/` nos mesmos caminhos relativos a agentes e KB.
...
|-------|-----------------|-----------|
| 1 | `@.claude/agents/communication/icatu-slide-planner.md` | Produz **slide-map** em Markdown: tipos `slide--icatu-*`, chunks, layout, acentos — **sem HTML**. |
| 2 | `@.claude/agents/communication/icatu-slide-builder.md` + `@.claude/kb/icatu-slides/` | Gera o **ficheiro HTML** alinhado ao mapa e à KB. |
| 3 | `@.claude/agents/communication/icatu-slide-reviewer.md` + ficheiro HTML | **Relatório** só leitura: paleta, marca, motor, pt-BR, checklist PDFs. |
| 4 (se preciso) | `@.claude/agents/communication/icatu-slide-fixer.md` + relatório + HTML | **Edits mínimos** (hex, acentos, eventos, etc.). **Não** regerar o deck. |
| 5 | `icatu-slide-reviewer` **outra vez** no mesmo HTML | Confirmar PASS ou iterar até aceitável. |

**Claude Code:** trocar o prefixo `.claude/` por `.claude/` nos `@`.  
...
| Recurso | Caminho |
|---------|---------|
| KB Icatu (paleta, tipos, template) | `.claude/kb/icatu-slides/` |
| PDFs e logo | `templates/ppts/ICATU/` |
| Roteamento geral de agentes | `.claude/commands/core/router.md` |
