...
| Caminho | Função |
|---------|--------|
| **`/DATALAKE/dags`** | Ficheiros `.py` das DAGs que o **Airflow** carrega (`AIRFLOW__CORE__DAGS_FOLDER`). Fica sob **`/DATALAKE`**, ao lado lógico de **`/DATALAKE/workspace`** (DSV/PRD), separado da documentação em **`.claude/kb`**. |
