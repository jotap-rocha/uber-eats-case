...
Ponto de entrada quando trabalhas com **`.claude/kb`** ou `@`. **Este directorio** (`.claude/kb/migration-hub/`) concentra a documentação narrativa da migração e da stack; **código executável** (scripts de infra, dbt): **`/DATALAKE/workspace/jp/platform/`** (`scripts/`, `dbt/`).
