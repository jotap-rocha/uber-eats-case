...
Structured Streaming is a scalable and fault-tolerant stream processing engine built on the Spark SQL engine. You can express your streaming computation the same way you would express a batch computation on static data. The Spark SQL engine will take care of running it incrementally and continuously and updating the final result as streaming data continues to arrive. You can use the [Dataset/DataFrame API](../sql-programming-guide.html) in Scala, Java, Python or R to express streaming aggregations, event-time windows, stream-to-batch joins, etc. The computation is executed on the same optimized Spark SQL engine. Finally, the system ensures end-to-end exactly-once fault-tolerance guarantees through checkpointing and Write-Ahead Logs. In short, _Structured Streaming provides fast, scalable, fault-tolerant, end-to-end exactly-once stream processing without the user having to reason about streaming._
...
In this guide, we are going to walk you through the programming model and the APIs. We are going to explain the concepts mostly using the default micro-batch processing model, and then [later](about:blank/performance-tips.html#continuous-processing) discuss Continuous Processing model. First, let’s start with a simple example of a Structured Streaming query - a streaming word count.
...
Let’s say you want to maintain a running word count of text data received from a data server listening on a TCP socket. Let’s see how you can express this using Structured Streaming. You can see the full code in [Python](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/python/sql/streaming/structured_network_wordcount.py)/[Scala](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/scala/org/apache/spark/examples/sql/streaming/StructuredNetworkWordCount.scala)/[Java](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/java/org/apache/spark/examples/sql/streaming/JavaStructuredNetworkWordCount.java)/[R](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/r/streaming/structured_network_wordcount.R). And if you [download Spark](https://spark.apache.org/downloads.html), you can directly [run the example](about:blank/index.html#running-the-examples-and-shell). In any case, let’s walk through the example step-by-step and understand how it works. First, we have to import the necessary classes and create a local SparkSession, the starting point of all functionalities related to Spark.
...
To actually execute this example code, you can either compile the code in your own [Spark application](about:blank/quick-start.html#self-contained-applications), or simply [run the example](about:blank/index.html#running-the-examples-and-shell) once you have downloaded Spark. We are showing the latter. You will first need to run Netcat (a small utility found in most Unix-like systems) as a data server by using
...
Furthermore, this model naturally handles data that has arrived later than expected based on its event-time. Since Spark is updating the Result Table, it has full control over updating old aggregates when there is late data, as well as cleaning up old aggregates to limit the size of intermediate state data. Since Spark 2.1, we have support for watermarking which allows the user to specify the threshold of late data, and allows the engine to accordingly clean up old state. These are explained later in more detail in the [Window Operations](about:blank/apis-on-dataframes-and-datasets.html#window-operations-on-event-time) section.
...
* Source: Rate Source
  ...
  * Fault-tolerant: Yes
  * Notes: 
* Source: Rate Per Micro-Batch Source (format: rate-micro-batch)
  ...
  * Notes: 
* Source: Kafka Source
  * Options:         See the Kafka Integration Guide.    
  * Fault-tolerant: Yes
  * Notes: 
* Source: 
  * Options: 
...
Imagine our [quick example](about:blank/getting-started.html#quick-example) is modified and the stream now contains lines along with the time when the line was generated. Instead of running word counts, we want to count words within 10 minute windows, updating every 5 minutes. That is, word counts in words received between 10 minute windows 12:00 - 12:10, 12:05 - 12:15, 12:10 - 12:20, etc. Note that 12:00 - 12:10 means data that arrived after 12:00 but before 12:10. Now, consider a word that was received at 12:07. This word should increment the counts corresponding to two windows 12:00 - 12:10 and 12:05 - 12:15. So the counts will be indexed by both, the grouping key (i.e. the word) and the window (can be calculated from the event-time).

The result tables would look something like the following.

![Window Operations](/img/structured-streaming-window.png)

Since this windowing is similar to grouping, in code, you can use `groupBy()` and `window()` operations to express windowed aggregations. You can see the full code for the below examples in [Python](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/python/sql/streaming/structured_network_wordcount_windowed.py)/[Scala](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/scala/org/apache/spark/examples/sql/streaming/StructuredNetworkWordCountWindowed.scala)/[Java](https://github.com/apache/spark/blob/v4.0.1/examples/src/main/java/org/apache/spark/examples/sql/streaming/JavaStructuredNetworkWordCountWindowed.java).
...
Some sinks (e.g. files) may not supported fine-grained updates that Update Mode requires. To work with them, we have also support Append Mode, where only the _final counts_ are written to sink. This is illustrated below.
...
*   **Output mode must be Append or Update.** Complete mode requires all aggregate data to be preserved, and hence cannot use watermarking to drop intermediate state. See the [Output Modes](#output-modes) section for detailed explanation of the semantics of each output mode.
...
Note that stream-static joins are not stateful, so no state management is necessary. However, a few types of stream-static outer joins are not yet supported. These are listed at the [end of this Join section](#support-matrix-for-joins-in-streaming-queries).
...
Outer joins have the same guarantees as [inner joins](#semantic-guarantees-of-stream-stream-inner-joins-with-watermarking) regarding watermark delays and whether data will be dropped or not.
...
Semi joins have the same guarantees as [inner joins](#semantic-guarantees-of-stream-stream-inner-joins-with-watermarking) regarding watermark delays and whether data will be dropped or not.
...
* Left Input: Left Outer
  * Right Input: Supported, not stateful
  * Join Type: 
* Left Input: Right Outer
  * Right Input: Not supported
  * Join Type: 
* Left Input: Full Outer
  * Right Input: Not supported
  * Join Type: 
* Left Input: Left Semi
  * Right Input: Supported, not stateful
  * Join Type: 
* Left Input: Static
  * Right Input: Stream
...
* Left Input: Left Outer
  * Right Input:       Conditionally supported, must specify watermark on right + time constraints for correct      results, optionally specify watermark on left for all state cleanup    
  * Join Type: 
* Left Input: Right Outer
  * Right Input:       Conditionally supported, must specify watermark on left + time constraints for correct      results, optionally specify watermark on right for all state cleanup    
  * Join Type: 
* Left Input: Full Outer
  * Right Input:       Conditionally supported, must specify watermark on one side + time constraints for correct      results, optionally specify watermark on the other side for all state cleanup    
  * Join Type: 
* Left Input: Left Semi
  * Right Input:       Conditionally supported, must specify watermark on right + time constraints for correct      results, optionally specify watermark on left for all state cleanup    
  * Join Type: 
...
*   Few types of outer joins on streaming Datasets are not supported. See the [support matrix in the Join Operations section](#support-matrix-for-joins-in-streaming-queries) for more details.
...
* Config Name: spark.sql.streaming.stateStore.rocksdb.compactOnCommit
  * Description: Whether we perform a range compaction of RocksDB instance for commit operation
  * Default Value: False
* Config Name: spark.sql.streaming.stateStore.rocksdb.changelogCheckpointing.enabled
  * Description: Whether to upload changelog instead of snapshot during RocksDB StateStore commit
  * Default Value: False
* Config Name: spark.sql.streaming.stateStore.rocksdb.blockSizeKB
  * Description: Approximate size in KB of user data packed per block for a RocksDB BlockBasedTable, which is a RocksDB's default SST file format.
  * Default Value: 4
* Config Name: spark.sql.streaming.stateStore.rocksdb.blockCacheSizeMB
  * Description: The size capacity in MB for a cache of blocks.
  * Default Value: 8
* Config Name: spark.sql.streaming.stateStore.rocksdb.lockAcquireTimeoutMs
  * Description: The waiting time in millisecond for acquiring lock in the load operation for RocksDB instance.
  * Default Value: 60000
* Config Name: spark.sql.streaming.stateStore.rocksdb.maxOpenFiles
  * Description: The number of open files that can be used by the RocksDB instance. Value of -1 means that files opened are always kept open. If the open file limit is reached, RocksDB will evict entries from the open file cache and close those file descriptors and remove the entries from the cache.
  * Default Value: -1
* Config Name: spark.sql.streaming.stateStore.rocksdb.resetStatsOnLoad
  * Description: Whether we resets all ticker and histogram stats for RocksDB on load.
  * Default Value: True
* Config Name: spark.sql.streaming.stateStore.rocksdb.trackTotalNumberOfRows
  * Description: Whether we track the total number of rows in state store. Please refer the details in Performance-aspect considerations.
  * Default Value: True
* Config Name: spark.sql.streaming.stateStore.rocksdb.writeBufferSizeMB
  * Description: The maximum size of MemTable in RocksDB. Value of -1 means that RocksDB internal default values will be used
  * Default Value: -1
* Config Name: spark.sql.streaming.stateStore.rocksdb.maxWriteBufferNumber
  * Description: The maximum number of MemTables in RocksDB, both active and immutable. Value of -1 means that RocksDB internal default values will be used
  * Default Value: -1
* Config Name: spark.sql.streaming.stateStore.rocksdb.boundedMemoryUsage
  * Description: Whether total memory usage for RocksDB state store instances on a single node is bounded.
  * Default Value: false
* Config Name: spark.sql.streaming.stateStore.rocksdb.maxMemoryUsageMB
  * Description: Total memory limit in MB for RocksDB state store instances on a single node.
  * Default Value: 500
* Config Name: spark.sql.streaming.stateStore.rocksdb.writeBufferCacheRatio
  * Description: Total memory to be occupied by write buffers as a fraction of memory allocated across all RocksDB instances on a single node using maxMemoryUsageMB.
  * Default Value: 0.5
* Config Name: spark.sql.streaming.stateStore.rocksdb.highPriorityPoolRatio
  * Description: Total memory to be occupied by blocks in high priority pool as a fraction of memory allocated across all RocksDB instances on a single node using maxMemoryUsageMB.
  * Default Value: 0.1
* Config Name: spark.sql.streaming.stateStore.rocksdb.allowFAllocate
  * Description: Allow the rocksdb runtime to use fallocate to pre-allocate disk space for logs, etc...  Disable for apps that have many smaller state stores to trade off disk space for write performance.
  * Default Value: true
* Config Name: spark.sql.streaming.stateStore.rocksdb.compression
  * Description: Compression type used in RocksDB. The string is converted RocksDB compression type through RocksDB Java API getCompressionType(). 
  * Default Value: lz4
...
Once you have defined the final result DataFrame/Dataset, all that is left is for you to start the streaming computation. To do that, you have to use the `DataStreamWriter` ([Python](about:/api/python/reference/pyspark.ss/api/pyspark.sql.streaming.DataStreamWriter.html#pyspark.sql.streaming.DataStreamWriter)/[Scala](https://spark.apache.org/api/scala/org/apache/spark/sql/streaming/DataStreamWriter.html)/[Java](https://spark.apache.org/api/java/org/apache/spark/sql/streaming/DataStreamWriter.html) docs) returned through `Dataset.writeStream()`. You will have to specify one or more of the following in this interface.
...
* Query Type: Queries with aggregation
  * Aggregation on event-time with watermark
  * Supported Output Modes: Append, Update, Complete
  * Notes:         Append mode uses watermark to drop old aggregation state. But the output of a        windowed aggregation is delayed the late threshold specified in withWatermark() as by        the modes semantics, rows can be added to the Result Table only once after they are        finalized (i.e. after watermark is crossed). See the        Late Data section for more details.                Update mode uses watermark to drop old aggregation state.                Complete mode does not drop old aggregation state since by definition this mode        preserves all data in the Result Table.    
* Query Type: Other aggregations
  * Complete, Update
  * Supported Output Modes:         Since no watermark is defined (only defined in other category),        old aggregation state is not dropped.                Append mode is not supported as aggregates can update thus violating the semantics of        this mode.    
  * Notes: 
* Query Type: Queries with mapGroupsWithState
  * Update
  * Supported Output Modes:       Aggregations not allowed in a query with mapGroupsWithState.    
  * Notes: 
* Query Type: Queries with flatMapGroupsWithState
  * Append operation mode
  * Supported Output Modes: Append
  * Notes:       Aggregations are allowed after flatMapGroupsWithState.    
* Query Type: Update operation mode
  * Update
  * Supported Output Modes:       Aggregations not allowed in a query with flatMapGroupsWithState.    
  * Notes: 
* Query Type: Queries with joins
  * Append
  * Supported Output Modes:         Update and Complete mode not supported yet. See the        support matrix in the Join Operations section         for more details on what types of joins are supported.      
  * Notes: 
* Query Type: Other queries
  * Append, Update
  * Supported Output Modes:       Complete mode not supported as it is infeasible to keep all unaggregated data in the Result Table.    
  * Notes: 
* Query Type: 
...
Some sinks are not fault-tolerant because they do not guarantee persistence of the output and are meant for debugging purposes only. See the earlier section on [fault-tolerance semantics](about:blank/getting-started.html#fault-tolerance-semantics). Here are the details of all the sinks in Spark.



* Sink: File Sink
  * Supported Output Modes: Append
  * Options:         path: path to the output directory, must be specified.        retention: time to live (TTL) for output files. Output files which batches were        committed older than TTL will be eventually excluded in metadata log. This means reader queries which read        the sink's output directory may not process them. You can provide the value as string format of the time. (like "12h", "7d", etc.)        By default it's disabled.                For file-format-specific options, see the related methods in DataFrameWriter        (Python/Scala/Java/R).        E.g. for "parquet" format options see DataFrameWriter.parquet()
  * Fault-tolerant: Yes (exactly-once)
  * Notes: Supports writes to partitioned tables. Partitioning by time may be useful.
* Sink: Kafka Sink
  * Supported Output Modes: Append, Update, Complete
  * Options: See the Kafka Integration Guide
  * Fault-tolerant: Yes (at-least-once)
  * Notes: More details in the Kafka Integration Guide
* Sink: Foreach Sink
  * Supported Output Modes: Append, Update, Complete
  * Options: None
  * Fault-tolerant: Yes (at-least-once)
  * Notes: More details in the next section
* Sink: ForeachBatch Sink
  * Supported Output Modes: Append, Update, Complete
  * Options: None
...
* Sink: Console Sink
  * Supported Output Modes: Append, Update, Complete
  * Options:         numRows: Number of rows to print every trigger (default: 20)                truncate: Whether to truncate the output if too long (default: true)    
  * Fault-tolerant: No
  * Notes: 
* Sink: Memory Sink
  * Supported Output Modes: Append, Complete
  * Options: None
  * Fault-tolerant: No. But in Complete Mode, restarted query will recreate the full table.
  * Notes: Table name is the query name.
* Sink: 
  * Supported Output Modes: 
  * Options: 
  * Fault-tolerant: 
...
For more details, please check the docs for DataStreamReader ([Python](about:/api/python/reference/pyspark.ss/api/pyspark.sql.streaming.DataStreamReader.html#pyspark.sql.streaming.DataStreamReader)/[Scala](https://spark.apache.org/api/scala/org/apache/spark/sql/streaming/DataStreamReader.html)/[Java](https://spark.apache.org/api/java/org/apache/spark/sql/streaming/DataStreamReader.html) docs) and DataStreamWriter ([Python](about:/api/python/reference/pyspark.ss/api/pyspark.sql.streaming.DataStreamWriter.html#pyspark.sql.streaming.DataStreamWriter)/[Scala](https://spark.apache.org/api/scala/org/apache/spark/sql/streaming/DataStreamWriter.html)/[Java](https://spark.apache.org/api/java/org/apache/spark/sql/streaming/DataStreamWriter.html) docs).
...
* Trigger Type: unspecified (default)
  * Description:         If no trigger setting is explicitly specified, then by default, the query will be        executed in micro-batch mode, where micro-batches will be generated as soon as        the previous micro-batch has completed processing.    
* Trigger Type: Fixed interval micro-batches
...
* Trigger Type: One-time micro-batch(deprecated)
...
* Trigger Type: Available-now micro-batch
  * Description:         Similar to queries one-time micro-batch trigger, the query will process all the available data and then        stop on its own. The difference is that, it will process the data in (possibly) multiple micro-batches        based on the source options (e.g. maxFilesPerTrigger or maxBytesPerTrigger for file         source), which will result in better query scalability.                    This trigger provides a strong guarantee of processing: regardless of how many batches were                left over in previous run, it ensures all available data at the time of execution gets                processed before termination. All uncommitted batches will be processed first.            Watermark gets advanced per each batch, and no-data batch gets executed before termination                if the last batch advances the watermark. This helps to maintain smaller and predictable                state size and smaller latency on the output of stateful operators.                NOTE: this trigger will be deactivated when there is any source which does not support Trigger.AvailableNow.        Spark will perform one-time micro-batch as a fall-back. Check the above differences for a risk of fallback.    
* Trigger Type: Continuous with fixed checkpoint interval(experimental)
  * Description:         The query will be executed in the new low-latency, continuous processing mode. Read more        about this in the Continuous Processing section below.    
...
Spark supports reporting metrics using the [Dropwizard Library](about:blank/monitoring.html#metrics). To enable metrics of Structured Streaming queries to be reported as well, you have to explicitly enable the configuration `spark.sql.streaming.metricsEnabled` in the SparkSession.
...
All queries started in the SparkSession after this configuration has been enabled will report metrics through Dropwizard to whatever [sinks](about:blank/monitoring.html#metrics) have been configured (e.g. Ganglia, Graphite, JMX, etc.).

Recovering from Failures with Checkpointing
-------------------------------------------

In case of a failure or intentional shutdown, you can recover the previous progress and state of a previous query, and continue where it left off. This is done using checkpointing and write-ahead logs. You can configure a query with a checkpoint location, and the query will save all the progress information (i.e. range of offsets processed in each trigger) and the running aggregates (e.g. word counts in the [quick example](about:blank/getting-started.html#quick-example)) to the checkpoint location. This checkpoint location has to be a path in an HDFS compatible file system, and can be set as an option in the DataStreamWriter when [starting a query](#starting-streaming-queries).
...
* Option: asyncProgressTrackingEnabled
  * Value: true/false
  * Default: false
  * Description: enable or disable asynchronous progress tracking
* Option: asyncProgressTrackingCheckpointIntervalMs
  * Value: millisecond
  * Default: 1000
  * Description: the interval in which we commit offsets and completion commits
...
*   See and run the [Python](https://github.com/apache/spark/tree/v4.0.1/examples/src/main/python/sql/streaming)/[Scala](https://github.com/apache/spark/tree/v4.0.1/examples/src/main/scala/org/apache/spark/examples/sql/streaming)/[Java](https://github.com/apache/spark/tree/v4.0.1/examples/src/main/java/org/apache/spark/examples/sql/streaming)/[R](https://github.com/apache/spark/tree/v4.0.1/examples/src/main/r/streaming) examples.
    *   [Instructions](about:blank/index.html#running-the-examples-and-shell) on how to run Spark examples
