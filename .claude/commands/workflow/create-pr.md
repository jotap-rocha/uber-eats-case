...
Este fluxo trata **`.claude/`** como referência para `agents/`, `kb/`, `commands/` e `sdd/`. Se o PR alterar também **`.claude/`** ou **`.github/`**, não há checagem automática de alinhamento entre árvores; alinhe **manualmente** se o time exigir conteúdo equivalente.
...
[ ] CONTEXT.md / .cursorrules / `.claude/CURSOR.MD`: atualizados se mudou como se roda, depura ou usa agentes/comandos (veja Definition of Done em [`HOW_TO_USE_AGENTS.md`](../../../get_started/HOW_TO_USE_AGENTS.md) e `.claude/commands/core/router.md`)
```
