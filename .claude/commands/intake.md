...
**Arquivo canônico:** `.claude/commands/intake.md` (este documento).

**Fonte única de dados:** [`.claude/sdd/architecture/AGENT_ROUTER.yaml`](../sdd/architecture/AGENT_ROUTER.yaml) — hints, roster, contrato `intake_contract` e regras de síntese.
...
1. **Explorar** (opcional): `/intake O que falta no meu .claude/ comparado ao template?`
