...
description: |
  ...
  /reviewer-round e /reviewer-fixer. Estado em .claude/sdd/reviews/{RUN_ID}/.
---

# `/workflow-makeapp-review` — MakeApp Review (entrada)

**Arquivo canônico:** `.claude/commands/workflow-makeapp-review/workflow-makeapp-review.md` (este documento).

Este comando é só o **roteador**: execute os passos abaixo **na ordem**, um slash por objetivo.

## Passos

1. **`/reviewer-init <RUN_ID>`** — Cria `.claude/sdd/reviews/{RUN_ID}/`, manifest, stubs de lanes e log inicial.  
   Ex.: `/reviewer-init pr-1842`

2. **`/reviewer-lane <RUN_ID> <lane_id>`** — Uma **lane por invocação** (não misturar no mesmo chat).  
   Ordem recomendada (ver [`.claude/sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml`](../../sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml)):
...
Agentes: `.claude/agents/workflow-makeapp-review/` · Índice: [`README.md`](README.md)
