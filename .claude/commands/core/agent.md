...
2. consultar `.claude/commands/core/router.md`
3. selecionar o agente primário mais apropriado
4. abrir e seguir o arquivo markdown do agente selecionado em `.claude/agents/`
5. consultar a knowledge base relevante em `.claude/kb/`, quando aplicável
...
- Sempre consultar `.claude/commands/core/router.md` antes de responder
...
- Quando o projeto evoluir (novos agentes, novas prioridades), **atualizar** `.claude/commands/core/router.md` para refletir a mudança
...
### Etapa 1 — roteamento
Leia `.claude/commands/core/router.md` e determine:
...
Abra o arquivo do agente primário selecionado em `.claude/agents/`.
...
- consulte `.claude/kb/_index.yaml`
- carregue o domínio relevante em `.claude/kb/`
...
- agente primário
- KB relevante
- contexto do projeto em `.claude/CURSOR.MD` (se existir)
...
|----------|------------------|
| `.claude/commands/core/router.md` | Novos `.md` em `.claude/agents/`, mudança de prioridades ou de “agente expert” do repo |
| `.cursorrules`, `.claude/CURSOR.MD`, `CONTEXT.md` | Mudança de comandos, estrutura ou convenções usadas no dia a dia |
