---
name: sync-context
description: Sync project context to .claude/CURSOR.MD by analyzing codebase patterns and conventions
---

# Sync Context Command

Analyzes codebase and intelligently updates `.claude/CURSOR.MD` with current project context.
...
3. **Merges** with existing `.claude/CURSOR.MD` content
...
## `.claude/CURSOR.MD` template

Generated `.claude/CURSOR.MD` follows this structure:
...
```text
1. Read existing `.claude/CURSOR.MD`
   │
   ▼
...
6. Validate (dry-run or save)
   │
   ▼
7. Write updated `.claude/CURSOR.MD`
```
...
━━━━━━━━━━━━━━━━
`.claude/CURSOR.MD` updated successfully
```
...
git diff .claude/CURSOR.MD

# Commit with context
git add .claude/CURSOR.MD
git commit -m "chore: update .claude/CURSOR.MD with latest project structure"
```