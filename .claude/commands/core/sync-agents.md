...
name: sync-agents
description: |
  Mantém o índice local de agentes em `.claude/agents/README.md` alinhado com os
...
# Sync Agents — índice `.claude/agents/README.md`

Garante que a seção **«Índice local (este repositório)»** em `.claude/agents/README.md` reflete todos os agentes presentes em `.claude/agents/**/*.md`, com **Total: N agentes** correto e uma linha por agente (link + finalidade).
...
| Entrada | Regra |
|---------|--------|
| Ficheiros contados | `.claude/agents/**/*.md` |
...
Ou no Cursor: `Glob(".claude/agents/**/*.md")` e filtrar mentalmente `README` e `_template`.

### 2. Actualizar o README

1. Abrir `.claude/agents/README.md`.
...
```text
/sync-agents — lista todos os .md em .claude/agents exceto README e _template;
...
- Links no README: relativos ao directório `.claude/agents/` (ex.: `[nome](data-engineering/nome.md)`).
