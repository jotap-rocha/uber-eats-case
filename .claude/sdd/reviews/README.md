...
Definição canônica: `.claude/commands/workflow-makeapp-review/workflow-makeapp-review.md` (entrada) e `reviewer-*.md` (atalhos em `commands/review/reviewer-*.md` apontam para `workflow-makeapp-review/`).
...
Ver [`.claude/sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml`](../architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml).

## Relação com SDD

O pipeline **Brainstorm → Define → Design → Build → Ship** continua em `.claude/commands/workflow/`.  
