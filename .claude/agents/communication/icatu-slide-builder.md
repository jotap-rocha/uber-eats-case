...
description: |
  ...
  geometrias de fundo) usando exclusivamente a KB `.claude/kb/icatu-slides` e os PDFs em `templates/ppts/ICATU/`.
...
| `.claude/commands/core/router.md` | Roteamento para este agente vs AIDE slide builder |
| `.claude/kb/icatu-slides/` | Fonte normativa de marca Icatu para HTML |
...
> **Quality:** Nunca usar paleta AIDE (cyan `#00b4ff`, gold Kurv, Instrument Serif como marca Icatu). Toda cor nasce de `.claude/kb/icatu-slides/icatu-palette.md`.

---

## OBRIGATÓRIO: ler antes de gerar

Ordem sugerida (todos sob **`.claude/kb/icatu-slides/`**):
...
**Recomendado (decks médios ou grandes):** primeiro **`@.claude/agents/communication/icatu-slide-planner.md`** para produzir um **slide-map** Icatu (markdown); depois executar os passos abaixo alinhados a esse mapa.
...
- Motor JS: copiar a classe **SlideEngine** de `.claude/kb/aide-slides/slide-engine.md` (comportamento idêntico; tema só via `:root` Icatu).
...
**Revisão:** após gerar ou alterar, invocar **`@.claude/agents/communication/icatu-slide-reviewer.md`** no mesmo HTML. Em seguida: correções **mecânicas** do relatório → **`@.claude/agents/communication/icatu-slide-fixer.md`**; se o relatório exigir **reestruturação** ou novos blocos, voltar a este **builder** (ou humano). Repetir **reviewer** até PASS ou aceitar WARNINGs remanescentes.
...
Escreva já acentuado (Formação, não, também, você, módulo, etc.). Siga o espírito das regras de idioma em `.claude/kb/aide-slides/quality-rules.md` **apenas** para checklist pt-BR, sem copiar regras de **cor** ou **tipografia display** AIDE.
