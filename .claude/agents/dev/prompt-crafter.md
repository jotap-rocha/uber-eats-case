...
6. **Agent Map (obrigatório):** Para **cada** fase 🔴🟡🟢, indicar **qual agente** (caminho `@.claude/agents/.../*.md`) o executor deve carregar ao tratar daquela tarefa. Produzir:
...
Use caminhos completos: @.claude/agents/<pasta>/<nome>.md (alinhado a HOW_TO_USE_AGENTS.md no repo).
-->

| Etapa / tipo de trabalho | Agente sugerido | Caminho para @ no Cursor |
|--------------------------|-----------------|---------------------------|
| {ex.: decisão de arquitetura} | {nome curto} | `@.claude/agents/...` |
| ... | ... | ... |
...
### 🔴 RISKY (Do First)
{Each line starts with @.claude/agents/... when a specialist is required}
...
[ ] Each task line that needs delegation starts with @.claude/agents/... (not HTML comments only)
```
