...
description: |
  ...
  sob .claude/sdd/data-pipeline-reviews/{RUN_ID}/. Não executa análise profunda.
...
> **Domain:** `.claude/sdd/data-pipeline-reviews/` · `WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml`  
...
2. **Paths** — `root = .claude/sdd/data-pipeline-reviews/{RUN_ID}/`, `lanes/ = root/lanes/`.
3. Copiar template `.claude/sdd/data-pipeline-reviews/_template/REVIEW_MANIFEST.yaml`, substituindo `REPLACE_RUN_ID`, `REPLACE_ISO8601` e `{BASE}`.
