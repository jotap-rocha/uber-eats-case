...
- `RUN_ID` (pasta existente sob `.claude/sdd/reviews/{RUN_ID}/`)
...
(Definição única: [`.claude/sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml`](../../sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml).)

---

## Process (ordem fixa)

1. **Read** [`.claude/sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml`](../../sdd/architecture/WORKFLOW_MAKEAPP_REVIEW_CONTRACTS.yaml) — extrair só o bloco `lanes:` entry para `lane_id` (globs, kb_entry, checklist_spec, shallow_checks) e `paths.summary_filename`.
2. **Read** `.claude/sdd/reviews/{RUN_ID}/REVIEW_MANIFEST.yaml` — não ler outros runs.
...
6. **Escrever** `.claude/sdd/reviews/{RUN_ID}/lanes/{lane_id}.md`:
...
8. **Se `lane_id` é `ui_ux`** — **Write** `.claude/sdd/reviews/{RUN_ID}/{summary_filename}` (ex.: `REVIEW_SUMMARY.md`):
