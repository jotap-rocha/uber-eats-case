...
| Arquivo | Finalidade |
|---------|------------|
| `.clauderules` | Regras carregadas automaticamente na raiz do projeto |
...
| `.claude/commands/core/router.md` | Roteamento de agentes |
| `.claude/kb/migration-hub/` | Fonte narrativa do ambiente e da migração |
...
**KB:** `.claude/kb/migration-hub/spark/PADRAO-SPARK-SUBMIT-PROJETO-PRD.md`  
**Layout:** `.claude/kb/design-pipelines-patterns/patterns/spark-submit-yarn-config.md`
...
Registo canónico do domínio: `.claude/kb/_index.yaml` → `domains.migration-hub`.

| Prioridade | Caminho | Uso |
|------------|---------|-----|
| 1 | `.claude/kb/migration-hub/index.md` | Índice e ordem de leitura |
| 2 | `.claude/kb/migration-hub/planning.md` | Fases, roadmap migração |
| 3 | `.claude/kb/migration-hub/migration/index.md` | Domínio migração |
| 4 | `.claude/kb/migration-hub/naming_conventions.md` | Identidades Airflow, paths |
| 5 | `.claude/kb/migration-hub/infra/STATUS-AMBIENTE.md` | Estado das máquinas |
| 6 | `.claude/kb/migration-hub/infra/ssh_configuration.md` | Malha SSH **311/314**, `rsync` |
| 7 | `.claude/kb/migration-hub/architecture.md`, `cluster_topology.md`, `inventory.md` | Desenho e inventário |
| 8 | `.claude/kb/migration-hub/team.md` | Equipa / responsabilidades (quando aplicável) |
| 9 | `/DATALAKE/workspace/jp/platform/scripts/infra/validate_cluster.sh` | Validação em lote (311, utilizador `hadoop`) |

**Ponte curta:** `.claude/kb/migration/index.md` (lista de links).

**Spark — execução em projetos PRD:**

- `.claude/kb/migration-hub/spark/PADRAO-SPARK-SUBMIT-PROJETO-PRD.md` — **padrão obrigatório** `config/spark.conf`
- `.claude/kb/migration-hub/spark/01-spark-3.5.2.md`, `CHECKLIST-BOAS-PRATICAS.md`

**KB técnica complementar:**

- `.claude/kb/spark/` — PySpark, APIs, referências longas.
- `.claude/kb/design-pipelines-patterns/` — árvore de pastas + `patterns/spark-submit-yarn-config.md`
- `.claude/kb/dev-python-engineering/` — Python dados, performance.
...
1. Abrir `.claude/kb/migration-hub/index.md` e seguir a tabela “Por onde começar”.
...
3. Mencionar firewall/portas quando propor novos serviços (alinhado a `.clauderules`).
...
4. Para mudanças que alterem ambiente real: lembrar atualização de `inventory.md` / `architecture.md` / `planning.md` conforme `.clauderules`.
...
- **kb-architect:** estruturação de novos domínios em `.claude/kb/`, não troubleshooting de cluster.
