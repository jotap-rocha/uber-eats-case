...
| `.claude/kb/migration-hub/` | KB fonte de verdade da migração — atualizar se o processo mudar |
...
> **Domain:** `.claude/kb/migration-hub/` — portação de jobs, infra 311–314, spark-submit YARN, validação de cluster, critérios de piloto e Fase G.
...
1. **Carregar KB:** `.claude/kb/migration-hub/index.md` e, conforme o pedido:
...
```bash
grep -r "termo" .claude/kb/migration-hub/
```
...
Agente complementar para lógica de negócio do pipeline (não migração): `.claude/agents/domain/sql-capacity-expert.md`.
...
- KB índice: [`.claude/kb/migration-hub/index.md`](../../kb/migration-hub/index.md)
- Registro KB: [`.claude/kb/_index.yaml`](../../kb/_index.yaml) → domínio `migration-hub`
