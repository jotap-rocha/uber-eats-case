...
1. **Ler** `.claude/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml` → `sdd_design_injection.file_manifest_hints`
2. **Incluir** no manifest **todos** os componentes aplicáveis (Teams, quarentena, drift, sentinela, contrato, testes)
3. **Anexar** `.claude/sdd/templates/PIPELINE_OPS_DESIGN_APPENDIX.md` como secção `## Design operacional — componentes mandatórios`
