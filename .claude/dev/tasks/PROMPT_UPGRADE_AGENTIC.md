...
- **Não alterar:** `agents/domain/*-expert.md`, `CONTEXT.md`, `README.md`, `.clauderules`, `dev/tasks/PROMPT_*.md` locais (exceto bootstrap `PROMPT_INIT_*`, `PROMPT_HELLO_WORLD`)
...
- [ ] **Comandos** (`.claude/commands/`): para cada conflito, unir instruções novas do template sem remover handoffs ou RUN_ID específicos do projeto
- [ ] **Agentes** (`.claude/agents/`): acrescentar seções/KB links do template; preservar exemplos e escopo local no corpo do agente destino
- [ ] **KB** (`.claude/kb/`): copiar domínios/arquivos novos; em `index.md` conflitantes, mesclar tabelas de entrada (não duplicar linhas)
- [ ] **Regras** (`.claude/rules/`): acrescentar regras `.mdc` novas; em conflito, concatenar blocos distintos ou escolher o mais restritivo para segurança
