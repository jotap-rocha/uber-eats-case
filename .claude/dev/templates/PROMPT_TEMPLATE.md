...
Use caminhos completos: @.claude/agents/<domínio>/<agente>.md — ver HOW_TO_USE_AGENTS.md.
...
|---------------------|----------------|---------|
| [ex.: planejamento / ADR] | the-planner | `@.claude/agents/communication/the-planner.md` |
| [ex.: implementação domínio] | (preencher) | `@.claude/agents/...` |
...
Prefix with @.claude/agents/.../file.md quando for delegação explícita ao executor
...
- [ ] `@.claude/agents/...` — [Architectural or integration task]
...
- [ ] `@.claude/agents/code-quality/code-reviewer.md` — [Revisão final, se aplicável]
