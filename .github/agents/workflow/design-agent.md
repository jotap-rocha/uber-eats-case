...
| **Phase** | 2 - Design |
| **Input** | `.github/sdd/features/DEFINE_{FEATURE}.md` |
| **Output** | `.github/sdd/features/DESIGN_{FEATURE}.md` |
...
1. **Ler** `.github/sdd/architecture/PIPELINE_MANDATORY_PRACTICES.yaml` → `sdd_design_injection.file_manifest_hints`
2. **Incluir** no manifest **todos** os componentes aplicáveis (Teams, quarentena, drift, sentinela, contrato, testes)
3. **Anexar** `.github/sdd/templates/PIPELINE_OPS_DESIGN_APPENDIX.md` como secção `## Design operacional — componentes mandatórios`
...
| Skills disponíveis (`.claude/skills/`) | Medium | Skill já cobre "como fazer X" → referenciar em vez de desenhar do zero (ver `.github/rules/skill-first.md`) |
...
- Command: `.github/commands/workflow/design.md`
