...
6. **Agent Map (obrigatório):** Para **cada** fase 🔴🟡🟢, indicar **qual agente** (caminho `@.github/agents/.../*.md`) o executor deve carregar ao tratar daquela tarefa. Produzir:
...
```
.github/dev/tasks/PROMPT_{FEATURE_NAME}.md
```
...
PROMPT READY
============
File: .github/dev/tasks/PROMPT_{NAME}.md
...
Or review first:
  Read .github/dev/tasks/PROMPT_{NAME}.md
```
...
Use caminhos completos: @.github/agents/<pasta>/<nome>.md (alinhado a HOW_TO_USE_AGENTS.md no repo).
-->

| Etapa / tipo de trabalho | Agente sugerido | Caminho para @ no Cursor |
|--------------------------|-----------------|---------------------------|
| {ex.: decisão de arquitetura} | {nome curto} | `@.github/agents/...` |
| ... | ... | ... |
...
### 🔴 RISKY (Do First)
{Each line starts with @.github/agents/... when a specialist is required}
...
[ ] Each task line that needs delegation starts with @.github/agents/... (not HTML comments only)
```
