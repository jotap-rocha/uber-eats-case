...
- `RUN_ID` existente sob `.github/sdd/data-pipeline-reviews/{RUN_ID}/`.
...
Definição única: [`.github/sdd/architecture/WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml`](../../sdd/architecture/WORKFLOW_DATA_PIPELINE_REVIEW_CONTRACTS.yaml).

## Process

1. Ler o contrato e extrair só a entrada da lane: `kb_entry`, `checklist_spec`, `scope_globs`, `shallow_checks`, `finding_prefix`.
2. Ler `.github/sdd/data-pipeline-reviews/{RUN_ID}/REVIEW_MANIFEST.yaml`.
