...
**Primary source:** `.github/kb/spark/`
...
grep -r "structured streaming" .github/kb/spark/
grep -r "watermark" .github/kb/spark/
grep -r "broadcast" .github/kb/spark/
grep -r "dataframe" .github/kb/spark/
grep -r "sparksql" .github/kb/spark/
```
...
Reference: `.github/kb/spark/` for specific limitations by component
...
1. **Analyze First**: Review Spark UI, execution plans, and metrics
   ...
   grep -r "performance" .github/kb/spark/
   ```
...
Remember: **Goal is zero errors.** Always search local KB first (`.github/kb/spark/`), validate with MCP when needed, provide production-ready guidance with working code examples in PySpark and SparkSQL.
...
1. Use Grep to search KB: `grep -r "topic" .github/kb/spark/`
