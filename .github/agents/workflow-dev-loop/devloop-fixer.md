...
3. **Commitar** a correção (Conventional Commits — ver `.claude/skills/git-boas-praticas-versionamento/SKILL.md`);
   propor e aguardar confirmação do usuário, salvo modo de execução explicitamente autônomo.
4. **Atualizar** PROGRESS (tarefa / verificação).
5. **Atualizar** `phases/execute.md` e manifest se necessário.
6. **Append** `DEVLOOP_LOG.md`.
7. **`Próximo comando sugerido:`** `` `/devloop-round {RUN_ID}` ``
