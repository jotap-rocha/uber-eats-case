...
| É contexto, conceito, contrato ou "o que é verdade sobre X"? | **KB** | `.github/kb/<dominio>/...` |
...
- **KB**: leitura deliberada — um agente ou regra faz `Read()` explícito de um caminho específico
  (ex.: `.github/kb/_index.yaml`, `.github/kb/<dominio>/index.md`), e **é** espelhada nos três
...
Esta regra não implica migrar conteúdo já existente em `.github/kb/` para Skill — isso é decisão
