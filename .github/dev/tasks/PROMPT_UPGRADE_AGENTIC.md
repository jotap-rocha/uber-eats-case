...
- **Não alterar:** `agents/domain/*-expert.md`, `CONTEXT.md`, `README.md`, `.githubrules`, `dev/tasks/PROMPT_*.md` locais (exceto bootstrap `PROMPT_INIT_*`, `PROMPT_HELLO_WORLD`)
...
- [ ] **Comandos** (`.github/commands/`): para cada conflito, unir instruções novas do template sem remover handoffs ou RUN_ID específicos do projeto
- [ ] **Agentes** (`.github/agents/`): acrescentar seções/KB links do template; preservar exemplos e escopo local no corpo do agente destino
- [ ] **KB** (`.github/kb/`): copiar domínios/arquivos novos; em `index.md` conflitantes, mesclar tabelas de entrada (não duplicar linhas)
- [ ] **Regras** (`.github/rules/`): acrescentar regras `.mdc` novas; em conflito, concatenar blocos distintos ou escolher o mais restritivo para segurança
