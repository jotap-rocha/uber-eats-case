...
- **Projecto `platform` (scripts + dbt):** réplica no mesmo path que no **311**: **`/DATALAKE/workspace/jp/platform`**. Documentação em **`.github/kb/migration-hub/`**. Pacotes pesados em **`/DATALAKE/downloads`**. Sincronizar com o nó de referência via `rsync` quando mudar; **não** substituir `/DATALAKE/opt` nem `/DATALAKE/var` do 314 por cópia do 311 (ver [`README.md`](/DATALAKE/workspace/jp/platform/README.md)).
